# Load item data from data dragon to file
...
for atlasCounter in range(10):
    ...
    atlasReq = requests.get(url=dataDragonBaseUrl+"cdn/" +
                            latestVersion+"/img/sprite/item"+str(atlasCounter)+".png", stream=True)
    # stop loop if request doesn't exist
    if atlasReq.status_code != 200:
        print("failed, stopping here")
        break
    # otherwise save results to file
    with open("items/item"+str(atlasCounter)+".png", "wb") as out_file:
...
for lang in languages:
    ...
    itemReq = requests.get(url=dataDragonBaseUrl+"cdn/" +
                           latestVersion+"/data/" + lang + "/item.json")
    ...
    with open("items/" + lang + ".data.json", 'w', encoding="utf-8") as outfile:
